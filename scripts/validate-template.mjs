...
async function validateComponentFrontmatter(pluginDir, pluginName, pluginManifest = null) {
  const dirsToCheck = [
    { dir: "rules", requiredKeys: ["description"], ext: (e) => [".md", ".mdc", ".markdown"].includes(e) },
    { dir: "skills", requiredKeys: ["name", "description"], isSkill: true },
    { dir: "agents", requiredKeys: ["name", "description"], ext: (e) => [".md", ".mdc", ".markdown"].includes(e) },
    { dir: "commands", requiredKeys: ["name", "description"], ext: (e) => [".md", ".mdc", ".markdown", ".txt"].includes(e) },
  ];

  for (const { dir: dirKey, requiredKeys, ext, isSkill } of dirsToCheck) {
    let checkDir = path.join(pluginDir, dirKey);
    if (pluginManifest && pluginManifest[dirKey]) {
      const manifestPath = typeof pluginManifest[dirKey] === "string" ? pluginManifest[dirKey] : pluginManifest[dirKey];
      if (typeof manifestPath === "string" && !manifestPath.startsWith("http")) {
        checkDir = path.resolve(pluginDir, manifestPath);
      }
    }
    if (!(await pathExists(checkDir))) continue;

    const files = await walkFiles(checkDir);
    for (const file of files) {
      const extName = path.extname(file).toLowerCase();
      const baseName = path.basename(file);
      if (isSkill && baseName !== "SKILL.md") continue;
      if (!isSkill && ext && !ext(extName)) continue;
      await validateFrontmatterFile(file, dirKey.slice(0, -1), requiredKeys, pluginName);
    }
  }
}
...
async function main() {
  ...
  for (const [index, entry] of marketplace.plugins.entries()) {
    ...
    for (const field of manifestFields) {
      ...
      const values = extractPathValues(value);
      for (const v of values) {
        await validateReferencedPath(pluginDir, field, v, entry.name);
      }
    }
    ...
    const hooksFromManifest = pluginManifest.hooks && typeof pluginManifest.hooks === "string";
    if (!hooksFromManifest && !(await pathExists(hooksPath))) {
      addWarning(`${entry.name}: no hooks/hooks.json file found (only needed when using hooks).`);
    }

    const mcpFromManifest = pluginManifest.mcpServers;
    const mcpPath = path.join(pluginDir, ".mcp.json");
    if (!mcpFromManifest && !(await pathExists(mcpPath))) {
