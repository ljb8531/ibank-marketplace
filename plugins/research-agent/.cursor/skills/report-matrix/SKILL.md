...
name: report-matrix
description: >
  리포트 Section 5~6을 생성한다. 7개 영역 통합 비교 매트릭스와 디자인 시스템 비교표를 작성한다.
  Deep Dive 산출물의 평가 결과를 구조화해 한눈에 비교 가능하게 한다. report-writer가 Step 3에서 호출한다.
...
$ARGUMENTS = output/reports/sections/02-deepdive.md 경로 또는 내용.

## 참조 데이터

- output/reports/sections/02-deepdive.md (7개 영역 평가 ◉/◎/○)
- output/reports/sections/01-asis.md (자사 평가)
- output/data/reference-codes.json
- output/branding/*.json (Inspire 디자인 시스템)
- output/data/project-brief.json (design_direction)

## 실행 절차

### Part A: Section 5 — 7개 영역 통합 비교 매트릭스
...
### Part B: Section 6 — 디자인 시스템 비교

output/branding/ 의 JSON을 Read해 비교표 구성.
...
output/reports/sections/03-matrix.md
