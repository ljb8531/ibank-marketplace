---
name: capture-google
description: 브라우저에서 Google에 접속해 실제 사용자처럼 검색어를 입력하고, 검색 결과·기사/블로그 링크 클릭 후 본문 수집, 필요 시 이미지 검색으로 참고 이미지를 스크린캡처·저장한다. Web Scout이 서비스별 역방향 웹 리서치(공신력 기사·리뷰·후기 수집) 시 사용. cursor-ide-browser 필수.
---

# capture-google

Web-Scout 소속. **도메인 목록 페이지를 크롤링하는 것이 아니라**, 해당 서비스/도메인에 대한 **역방향 웹 검색**을 수행한다. Google 사이트에 접속해 검색어를 입력하고, 검색 결과에서 기사·블로그·사용 후기 등의 링크를 따라 들어가 콘텐츠를 수집하며, 필요 시 이미지 검색 결과를 캡처해 워크스페이스에 저장한다.

## 역할

- Service Scout은 **서비스 공식 페이지**를 수집하고, UXUI Scout은 **해당 페이지의 시각·구조**를 수집한다.
- **capture-google**은 같은 서비스 목록을 기준으로 **웹에서 “이 서비스에 대한” 기사·리뷰·후기·참고 이미지**를 찾아 수집한다.
- 수집한 데이터에 대한 분석은 하지 않는다. 데이터를 구조화하여 저장·반환하는 것이 목적이다.

## 입력

- **query** (필수): Google 검색창에 입력할 검색어 (예: `"서비스명 리뷰"`, `"서비스명 사용 후기"`, `"서비스명 비교"`).
...
- **image_search** (선택, 기본 false): Google 이미지 검색을 수행할지 여부.
- **max_images** (선택, 기본 5): 이미지 검색 시 캡처할 참고 이미지 개수 상한 (스크린캡처 또는 저장).

## 가드레일

1. **반드시 cursor-ide-browser를 사용**한다. WebSearch·mcp_web_fetch만으로는 “Google 사이트에서 직접 검색하는” 동작을 구현할 수 없으므로, 브라우저로 Google에 접속해 입력·클릭·이동을 수행한다.
...
6. **이미지 검색** 시: Google 이미지 검색 페이지(`&tbm=isch` 또는 이미지 검색 URL)로 이동한 뒤, 결과 그리드 또는 개별 이미지를 **browser_take_screenshot**으로 캡처해 `research/web/{service_slug}/images/`(또는 `research/images/{service_slug}/`)에 저장한다. 필요 시 이미지 클릭 → 큰 이미지 뷰 → 해당 영역 스크린캡처로 참고용 이미지를 확보한다.
7. 모든 수집 기록을 `research/temp/web-scout-log.md`(또는 `capture-google-log.md`)에 추가하고, 필요 시 `research/web/_index.md`를 갱신한다.
8. 한 번에 과도한 요청을 하지 않는다. 검색 1회·링크 N개·이미지 M개 수준으로 제한하고, 필요 시 쿼리별로 스킬을 여러 번 호출한다.

## 실행 절차

### Step 1: Google 접속 및 검색

- `[browser_navigate]` url=`https://www.google.com` (또는 사용 환경에 맞는 Google 검색 URL)
- `[browser_lock]`
- `[browser_snapshot]` → 검색 입력창 요소 식별 (ref, 역할)
- `[browser_type]` 또는 `[browser_fill]`: element=검색창 설명, ref=해당 ref, text=query, submit=true (또는 Enter로 제출)
- 검색 결과 로딩 대기: 짧은 대기 후 `[browser_snapshot]`으로 결과 페이지 구조 확인

### Step 2: 검색 결과에서 링크·스니펫 추출

- `[browser_snapshot]` → 검색 결과 영역에서 링크(URL)·제목·스니펫 추출
- 추출한 목록을 정리해 반환용 `search_results` 배열에 담고, 필요 시 `research/web/{service_slug}/search_results_{query_slug}.md`에 저장
...
- 상위 **max_links**개 링크에 대해 (광고·동일 사이트 중복은 제외):
  ...
  - 다음 결과를 수집하려면 Google 검색 결과 페이지로 `[browser_navigate_back]` 또는 검색 결과 URL로 재이동
- 수집한 각 페이지의 **url, title, content_path**를 로그 및 반환 구조에 기록

### Step 4: 이미지 검색 (image_search가 true일 때)

- `[browser_navigate]` url=`https://www.google.com/search?q={encodeURIComponent(query)}&tbm=isch`
- `[browser_snapshot]` → 이미지 결과 그리드 또는 썸네일 영역 확인
- **max_images**개까지:
  - 이미지 영역을 **browser_take_screenshot**으로 캡처해 `research/web/{service_slug}/images/img_{순번}.png`에 저장하거나,
  - 썸네일/이미지를 클릭해 큰 이미지 뷰로 연 뒤, 해당 뷰를 스크린캡처해 저장
- 저장한 이미지 경로 목록을 반환용 `image_paths` 배열에 담음

### Step 5: 종료 및 로그

- `[browser_unlock]`
- `research/temp/web-scout-log.md`에 이번 호출 요약 추가 (query, service_slug, 수집 링크 수, 저장 경로, 이미지 수)
...
```json
{
  ...
  "image_paths": [
    "research/web/{service_slug}/images/img_01.png",
    "research/web/{service_slug}/images/img_02.png"
  ],
...
- **mcp_web_fetch**: 세부 페이지 본문 수집 시 1차 시도 (빠르고 안정적일 때)

## 부산물

- `research/web/{service_slug}/articles/*.md`: 수집한 기사·블로그·후기 본문
- `research/web/{service_slug}/images/*.png`: 이미지 검색으로 캡처한 참고 이미지
- `research/web/{service_slug}/search_results_{query_slug}.md`: (선택) 검색 결과 요약
